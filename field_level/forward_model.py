...
class Forward_Model:
    ...
    ### under construction
    def full_cubic_op(self, delk_L, delr_L, renormalize=True):
        G1k_L = self.kG1_L*delk_L
        G1r_L = self.batch_irfftn(G1k_L)
    ...
    def models(self, delk_L, biases, *vals):
        if  'gauss' in self.model_name:
            ...
            if 'lin' in self.model_name:
                ...
                G2r_L = self.G2r(delk_L)
                weight_lin = delr_L
                weight_quad = 0.5*b2 * d2r_L +  bG2 * G2r_L
